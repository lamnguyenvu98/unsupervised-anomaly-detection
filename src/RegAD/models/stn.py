...
class STNModule(nn.Module):
    def __init__(self, in_num, block_index, stn_mode='affine'):
    ...
    def forward(self, x):
        ...
        if mode == 'affine':
            ...
            theta1 = Variable(torch.zeros([batch_size, 2, 3], dtype=torch.float32, device=x.device),
                              requires_grad=True)
            theta1 = theta1 + 0
            theta1[:, 0, 0] = 1.0
            theta1[:, 1, 1] = 1.0
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, stn_mode='rotation_scale'):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        fixthea1 = torch.from_numpy(np.linalg.inv(np.concatenate((theta1.detach().cpu().numpy(), tmp), axis=1))[:,:-1,:]).to(x.device)
        ...
        tmp = np.tile(np.array([0, 0, 1]), (x.shape[0], 1, 1)).astype(np.float32)
        fixthea2 = torch.from_numpy(np.linalg.inv(np.concatenate((theta2.detach().cpu().numpy(), tmp), axis=1))[:,:-1,:]).to(x.device)
        ...
        tmp = np.tile(np.array([0, 0, 1]), (x.shape[0], 1, 1)).astype(np.float32)
        fixthea3 = torch.from_numpy(np.linalg.inv(np.concatenate((theta3.detach().cpu().numpy(), tmp), axis=1))[:,:-1,:]).to(x.device)
...
def stn_net(resnet_type="resnet18", stn_mode='rotation_scale', pretrained=True, **kwargs):
    ...
    model = ResNet(BasicBlock, resnet[resnet_type], stn_mode, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls[resnet_type]), strict=False)
    return model
